...
/**
 * A helper object for interacting with the  chaincode.  Has functions for all of the query and invoke
...
/**
 * Create an account on the commercial DMV network.  The given enrollID will also be taken as the name for the
 * commercial trading account.
...
/**
 * Invoke the chaincode to create a new vehicle/title.
...
CPChaincode.prototype.createPaper = function(enrollID, paper, cb) {
    console.log(TAG, 'creating a new vehicle/title');

    // Paper information will be generated by the UI
    var createTitleRequest = {
        chaincodeID: this.chaincodeID,
        fcn: 'issueVehicleTitle',
        args: [JSON.stringify(paper)]
    };

    invoke(this.chain, enrollID, createTitleRequest, function(err, result) {
        if(err) {
            console.error(TAG, 'failed to create title:', err);
            return cb(err);
        }

        console.log(TAG, 'Created title successfully:', result.toString());
        cb(null, result);
    });
};
...
/**
 * Invoke the chaincode to transfer ownership of a vehicle title.
 * @param enrollID The user that the invoke should be submitted through.
...
CPChaincode.prototype.transferPaper = function(enrollID, paper, cb) {
    console.log(TAG, 'transferring a vehicle title');
    ...
    invoke(this.chain, enrollID, transferRequest, function(err, result) {
        if(err) {
            console.error(TAG, 'failed to transfer title:', err);
            return cb(err);
        }

        console.log(TAG, 'Transferred title successfully:', result.toString());
        cb(null, result);
    });
};

CPChaincode.prototype.terminateAsset = function(enrollID, asset, cb) {
    console.log(TAG, 'terminating asset');
    ...
    invoke(this.chain, enrollID, terminateRequest, function(err, result) {
        if(err) {
            console.error(TAG, 'failed to terminate asset:', err);
            return cb(err);
        }

        console.log(TAG, 'Terminated asset successfully:', result.toString());
        cb(null, result);
    });
};
...
CPChaincode.prototype.getPapers = function(enrollID, cb) {
    console.log(TAG, 'getting vehicle titles');
    ...
    query(this.chain, enrollID, getPapersRequest, function(err, papers) {
        ...
        if(err) {
            console.error(TAG, 'failed to getTitles:', err);
            return cb(err);
        }

        console.log(TAG, 'got titles');
